...
function Path:parent()
  return Path.new(vim.fn.fnamemodify(self:posix_path(), ":h"))
end
...
function Path:os_path()
  ...
  if Path.is_windows() then
    if vim.startswith(path, "/") then
      ...
      if drive then
        return string.format("%s:%s", drive, path:sub(drive:len() + 2):gsub("/", "\\"))
      end
    end
...
function Path:posix_path()
  ...
  if Path.is_windows() then
    ...
    if drive then
      return string.format("/%s/%s", drive:upper(), remaining:gsub("\\", "/"))
    end
    return util.select_n(1, path:gsub("\\", "/"))
  else
    return path
...
function Path:exists()
  return not not util.select_n(1, vim.uv.fs_stat(self:os_path()))
end

---@return uv.fs_stat.result|nil
function Path:stats()
  return util.select_n(1, vim.uv.fs_stat(self:os_path()))
end

---@return uv.fs_stat.result|nil
function Path:lstats()
  return util.select_n(1, vim.uv.fs_lstat(self:os_path()))
end
...
function Path:relative(ref)
  return vim.fs.relpath(self:posix_path(), Path.new(ref):posix_path())
end

---@return Path
function Path:join(...)
  return Path.new(vim.fs.joinpath(self:posix_path(), ...))
end
...
function Path:is_descendant_of(other)
  ...
  if #other_segments >= #self_segments then
    return false
  end
  for i = 1, #other_segments do
    if self_segments[i] ~= other_segments[i] then
      return false
    end
  end
  return true
...
---@param other string
---@return boolean
function Path:is_ancestor_of(other)
  return Path.new(other):is_descendant_of(self:posix_path())
end
