...
function Resolver:parse_buffer()
  ...
  for _, line in ipairs(lines) do
    ...
    while true do
      ...
      if not (parent.indentation >= indent_level and parents:size() > 1) then break end
      parents:pop()
    end
...
function Resolver:parse_actions()
  ...
  for ref_id, path in pairs(self.old_ref) do
    if not self.new_ref[ref_id] then table.insert(self.raw_act, { type = "delete", path = path }) end
  end
  ...
  for ref_id, old_path in pairs(self.old_ref) do
    (function()
      ...
      if util.if_any(paths, function(path) return path == old_path end) then
        util.tbl_each(
          util.tbl_filter(paths, function(path) return not (path == old_path) end),
          function(path) table.insert(self.raw_act, { type = "copy", src = old_path, dst = path }) end
        )
      else
        for i = 1, #paths - 1 do
...
local function is_automatic_child_move(child_act, parent_act)
  if child_act.type ~= "move" or parent_act.type ~= "move" then return false end
  if not Path.new(child_act.src):is_descendant_of(parent_act.src) then return false end
...
function Resolver:final_actions()
  ...
  for _, act in ipairs(self.raw_act) do
    (function()
      if (act.type == "move" or act.type == "copy") and act.src == act.dst then return end
  ...
  for _, act in ipairs(filtered) do
    ...
    if not is_redundant then table.insert(pruned, act) end
  end
...
---@return table
function Resolver:resolve() return self:parse_buffer():parse_actions():final_actions() end
